...
The project includes a database-backed restaurant management workflow. Reservations and menu items are stored in SQLite, and the **Manage** page lets staff create, locate, view, edit, and delete menu records through the API.

## Data model
---

- `customers` stores a unique customer identity (name and email).
- `reservations` stores booking details and links to `customers` through `customer_id`.
- `menu_items` stores dishes, categories, descriptions, prices, and chef-pick status.
- `newsletter_signups` stores unique newsletter subscribers.

The server initializes these relational tables automatically. Menu CRUD is available at `/api/menu` and through `manage.html`; reservation CRUD is available at `/api/reservations`.
...
Open `http://localhost:5000/` for the website and `http://localhost:5000/manage.html` for the menu management screen. The reservation form saves records to the database when the site is run through `npm start`.
