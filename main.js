...
document.addEventListener("DOMContentLoaded", () => {
  ...
  reservationForm.addEventListener("submit", async event => {
    ...
    // The production server shares the frontend origin; static previews keep the existing confirmation flow.
      ...
      try {
        const response = await fetch("/api/reservations", {
          ...
          body: JSON.stringify({
            ...
            date: dateInput.value,
            guests: Number(peopleSelect.value),
            requests: messageInput.value.trim()
          })
        });
        ...
        confirmationDetails.reservationId = result.reservationId;
      } catch (error) {
        if (submitButton) submitButton.disabled = false;
        showAlertMessage(reservationFeedback, "danger", error.message);
        return;
      }
    }
